...
function parseBlocks(rawLines: string[], prefix = ""): React.ReactNode[] {
  ...
  while (i < rawLines.length) {
    ...
    if (headingMatch) {
      ...
      elements.push(
        level === 1 ? (
          <h1 key={`${prefix}h1-${i}`} id={id} className={cn}>
            {inner}
          </h1>
        ) : level === 2 ? (
          <h2 key={`${prefix}h2-${i}`} id={id} className={cn}>
            {inner}
          </h2>
        ) : level === 3 ? (
          <h3 key={`${prefix}h3-${i}`} id={id} className={cn}>
            {inner}
          </h3>
        ) : level === 4 ? (
          <h4 key={`${prefix}h4-${i}`} id={id} className={cn}>
            {inner}
          </h4>
        ) : level === 5 ? (
          <h5 key={`${prefix}h5-${i}`} id={id} className={cn}>
            {inner}
          </h5>
        ) : (
          <h6 key={`${prefix}h6-${i}`} id={id} className={cn}>
            {inner}
          </h6>
        ),
      );
    ...
    if (trimmed.match(/^>\s*\[!(NOTE|TIP|WARNING|IMPORTANT|CAUTION)\]/i)) {
      ...
      elements.push(
        <div
          key={`${prefix}alert-${i}`}
          className={`md-alert md-alert--${alertType}`}
        >
          <div className="md-alert-title">
            <span className="md-alert-icon">
              {alertIcons[alertType] || "ℹ️"}
            </span>
    ...
    if (i + 1 < rawLines.length && rawLines[i + 1].trim().startsWith(": ")) {
      ...
      elements.push(
        <dl key={`${prefix}dl-${i}`} className="md-dl">
          {dlItems}
        </dl>,
      );
      continue;
    ...
    if (imgOnly) {
      elements.push(
        <figure key={`${prefix}fig-${i}`} className="md-figure">
          ...
          />
          {imgOnly[1] && (
            <figcaption className="md-figcaption">{imgOnly[1]}</figcaption>
          )}
        </figure>,
      );
      i++;
...
function collectListItems(
  ...
  while (i < lines.length) {
    ...
    if (nestedStart < lines.length) {
      ...
      if (
        nextIndent > indent &&
        itemRe.test(lines[nestedStart].trimStart().replace(/^\s*/, ""))
      ) {
    ...
    if (isTask) {
      nodes.push(
        <li
          key={i}
          className={`md-task-item${checked ? " md-task-checked" : ""}`}
        >
...
export function MarkdownTextRenderer({
  ...
  return <div className="md-rendered prose">{parseBlocks(lines)}</div>;
}
