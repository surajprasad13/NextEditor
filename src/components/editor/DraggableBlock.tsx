...
import {
  ArrowUp, ArrowDown, Trash2, GripVertical, Edit, Eye,
  Copy, Code, Layers, X,
} from 'lucide-react';
...
export function DraggableBlock({
  ...
  const renderComponent = (editable: boolean) => {
    const commonProps = {
      ...
      onChange: (p: any) => {
        ...
        if (!['Card', 'Accordion', 'StatsBlock', 'TestimonialRow'].includes(type)) {
          setViewMode('preview');
        }
      },
    };
    ...
    switch (type) {
      case 'HeroBlock':      return <HeroBlock {...commonProps} />;
      case 'Card':           return <Card {...commonProps} />;
      case 'ImageBlock':     return <ImageBlock {...commonProps} />;
      case 'TestimonialRow': return <TestimonialRow {...commonProps} />;
      case 'Accordion':      return <Accordion {...commonProps} />;
      case 'CalloutBlock':   return <CalloutBlock {...commonProps} />;
      case 'StatsBlock':     return <StatsBlock {...commonProps} />;
      case 'CodeBlock':      return <CodeBlock {...commonProps} />;
      case 'ButtonBlock':    return <ButtonBlock {...commonProps} />;
      case 'DividerBlock':   return <DividerBlock {...commonProps} />;
      case 'NavBlock':       return <NavBlock {...commonProps} />;
      case 'FooterBlock':    return <FooterBlock {...commonProps} />;
      case 'ColumnsBlock':   return <ColumnsBlock {...commonProps} />;
      case 'PricingBlock':   return <PricingBlock {...commonProps} />;
      case 'TextBlock':      return <TextBlock {...commonProps} />;
      case 'EmbedBlock':     return <EmbedBlock {...commonProps} isEditable={editable} />;
      default:
        return (
  ...
  const customCssBlock = hasCustomStyles ? (
    <style dangerouslySetInnerHTML={{
      __html: `
        ...
        ${styleSettings.hoverEffect === 'scale' ? `#canvas-node-inner-${id}:hover { transform: scale(1.03); }` : ''}
        ${styleSettings.hoverEffect === 'float' ? `#canvas-node-inner-${id}:hover { transform: translateY(-6px); }` : ''}
        ${styleSettings.hoverEffect === 'shadow' ? `#canvas-node-inner-${id}:hover { box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04) !important; }` : ''}
        ${styleSettings.hoverEffect === 'border-glow' ? `#canvas-node-inner-${id}:hover { border-color: var(--accent, #6366f1) !important; box-shadow: 0 0 12px rgba(99, 102, 241, 0.4) !important; }` : ''}
      `
    }} />
  ) : null;
  ...
  return (
    <div
      ref={blockRef}
      className={isPreviewMode ? `block-style-wrapper-preview ${styleSettings.customClass || ''}` : [
        'editor-block-wrapper',
        isSelected ? 'selected' : '',
        isDragOver ? 'drag-target' : '',
        viewMode === 'edit' ? 'in-edit-mode' : '',
      ].filter(Boolean).join(' ')}
      onClick={isPreviewMode ? undefined : (e) => { e.stopPropagation(); onSelect(); }}
    ...
    >
        ...
        <div className="block-toolbar">
          ...
          <div className="block-toolbar-actions">
            ...
            <button
              type="button"
              className="block-action-btn"
              onClick={(e) => { e.stopPropagation(); if (index < total - 1) onMove(index, index + 1); }}
            ...
            <button
              type="button"
              className={`block-action-btn ${viewMode === 'edit' ? 'active' : ''}`}
            ...
            <button
              type="button"
              className={`block-action-btn ${viewMode === 'json' ? 'active' : ''}`}
              onClick={viewMode === 'json' ? (e) => { e.stopPropagation(); setViewMode('preview'); } : openJsonEditor}
      ...
      <div className={isPreviewMode ? "" : "block-inner-content"}>
        {viewMode === 'json' && !isPreviewMode ? (
          <div className="block-json-editor" onClick={(e) => e.stopPropagation()}>
            <div className="json-editor-header">
              <span>
                {meta.icon} <strong>{type}</strong> — Raw JSON Props
              </span>
              <div style={{ display: 'flex', gap: '6px' }}>
                <button type="button" onClick={handleJsonSave} className="json-save-btn">
                  ✓ Apply
                </button>
                <button
                  type="button"
          ...
          <div 
            ...
            className={isPreviewMode ? styleSettings.customClass || '' : `block-style-wrapper ${styleSettings.customClass || ''}`}
